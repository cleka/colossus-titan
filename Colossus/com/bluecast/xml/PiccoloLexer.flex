...
%{
    ...
    char[] cdataBuffer;
    int cdataStart = 0, cdataLength = 0;
    ...
    private int prevEntityLexState = 0;
    private boolean returnEntityEndToken = true;
    private boolean isParamEntity = false;
    private int entityBaseDepth=0;
    private boolean isEntityBeingParsed = true;
    private boolean inCdataSection = false;

    private int baseState = YYINITIAL; // Used for reporting CDATA directly from lexer
    private int prevState = 0, piPrevState=0;
    private boolean isNamespaceDeclaration=false;
    private boolean entityWasSkipped=false;
    private int tagState = TAG_NS;
    private int tagStartState = TAG_START_NS;

    private boolean tokenizeInput=true;
    private StringStack tagStack = new StringStack(20);
    private boolean bypassPERefs=false;
    private boolean bypassGERefs=false;
    private boolean needsNormalization = false;
    private int tagType=0; // Piccolo.OPEN_TAG,EMPTY_TAG,CLOSE_TAG
    ...
    private String attributeValueType = null;
    ...
    private int numAttributesDefined = 0;
...
//        }
    ...
    private boolean isASCIINameStartChar(int c) {
        return ( (c>=0x61&&c<=0x7A)||(c==':')||(c>=0x41&&c<=0x5A)
               ||(c=='_')
    ...
    private boolean isASCIINameStartCharNS(int c) {
    ...
    private boolean isASCIINameChar(int c) {
        return ( (c>=0x61&&c<=0x7A)||(c==':')||(c>=0x41&&c<=0x5A)
                 ||(c == '.')||(c=='-')||(c=='_')
                 ||(c == 0xB7)
    ...
    // Same but excludes the colon
    private boolean isASCIINameCharNS(int c) {
        return ( (c>=0x61&&c<=0x7A)||(c>=0x41&&c<=0x5A)
                 ||(c == '.')||(c=='-')||(c=='_')
                 ||(c == 0xB7)
                 ||(c>=0x30&&c<=0x39)||(c>=0xC0&&c<=0xD6)
...
<TAG_VALUE_ENTITYREF,ENTITYREF,PI,PI_WS,TAG_NS,TAG_START_NS,TAG_VALUE,CLOSE_TAG,CLOSE_TAG_NS><<EOF>> {
